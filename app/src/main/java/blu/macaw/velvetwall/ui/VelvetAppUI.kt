...
// IMPORTS DAS TELAS (Verifique se estes imports estão corretos no seu projeto)
...
fun HistoryScreen(viewModel: MainViewModel) {
    ...
    // Estado para controlar qual item foi clicado e mostrar o Dialog
        ...
        if (history.isEmpty()) {
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    ...
                    Text("Nenhuma ameaça detectada.", color = Color.Gray)
                }
            }
        } else {
            LazyColumn {
                items(history) { log ->
                    HistoryItem(log) {
                        // Ao clicar, seleciona o item para abrir o Dialog
                        selectedLog = log
                    }
                }
            }
    ...
    // --- JANELA DE DECISÃO (Dialog) ---
    if (selectedLog != null) {
        AlertDialog(
            onDismissRequest = { selectedLog = null },
            containerColor = VelvetDark,
            title = {
                Text(text = selectedLog!!.number, color = RoyalCyan, fontWeight = FontWeight.Bold)
            },
            text = {
                Text("O que deseja fazer com este número?", color = Color.White)
            },
            confirmButton = {
                // Botão 1: PERMITIR (Whitelist)
                Button(
                    onClick = {
                        viewModel.allowFromHistory(selectedLog!!)
                        selectedLog = null
                    },
                    ...
                    Text("Permitir Sempre", color = VelvetBlack)
                }
            },
            dismissButton = {
                // Botão 2: BLOQUEAR (Blacklist)
                Row {
                    Button(
                        onClick = {
                            viewModel.blockFromHistory(selectedLog!!)
                            selectedLog = null
                        },
...
@Composable
fun HistoryItem(log: BlockedCallLog, onClick: () -> Unit) {
    ...
    Card(
        ...
        modifier = Modifier
            ...
            .clickable(onClick = onClick) // Clique aqui
    ) {
        Row(
            modifier = Modifier
                .padding(16.dp)
            ...
            Box(
                modifier = Modifier
                    .size(40.dp)
                    .background(Color(0xFF2D1F21), CircleShape), // Fundo avermelhado leve
